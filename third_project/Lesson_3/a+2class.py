...
while(word!="-1"):
    ...
    if (counter == len(word) - 1):
        match state:
            case 1:
                if(word[counter]=="0"): # machine in state 1 reading '0' go to the state 1
                    state= 1
                if (word[counter] == "1"): # machine in state '1' reading a go to the state 2
                    state= 2
                if (word[counter] != "0" and word[counter] != "1"): # when a letter don't belong to the alphabet
                    counter = counter - 1
                    break
            case 2:
                if (word[counter] == "0"): # machine in state 2 reading 'a' is not possible
                    counter = counter - 1
                    break
                if (word[counter] == "1"): # machine in state 2 reading 'b' go to the state 3
                    state=3
                if (word[counter] != "0" and word[counter] != "1"): # when a letter don't belong to the alphabet
                    counter = counter - 1
                    break
